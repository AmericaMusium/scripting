...
//    dllfunc(DLLFunc_Spawn, iEntity)
    ...
    //engfunc(EngFunc_SetSize, iEntity, Float: {-200.0, -200.0, -80.0}, Float:{200.0, 200.0, 1.0})
...
SpawnSavage(id)
{
    ...
    set_pev(TankEnt, pev_solid, SOLID_BBOX)
    ...
    set_pev(TankEnt, pev_maxs, Float:{200.0, 200.0, 2.0})
    set_pev(TankEnt, pev_mins, Float:{-200.0, -200.0, -80.0})
    // set_pev(TankEnt, pev_size, Float:{-200.0,-200.0, -80.0, 200.0, 200.0, 2.0})
    // engfunc(EngFunc_SetSize, TankEnt, Float: {-200.0, -200.0, -80.0}, Float:{200.0, 200.0, 2.0})
    entity_set_size(TankEnt, Float: {-200.0, -200.0, -80.0}, Float: {200.0, 200.0, 2.0})
   ...
   //  get_user_name(id, szName, charsmax(szName))
    ...
    new spawnP
    if( (spawnP = engfunc(EngFunc_FindEntityByString, -1, "classname", "info_player_allies")) != 0)
    {   
        new Float: yaOrigin[3]
        pev( spawnP, pev_origin, yaOrigin)
        set_pev(id, pev_origin, yaOrigin)
    }
...
public client_PreThink(id)
{
    ...
    if(pev_valid(g_idTank[id]) && pev_valid(id))
    ...
    set_hudmessage(130, 40, 10, 0.52, hudy , 0, 0.8, 0.4, 0.1, 0.1, -1)
	show_hudmessage(id, " _xxx %f", fAngB[0] )


    // retune velocity
    // set_pev( g_idTank[id], pev_velocity, Float:{ -150.0 , 150.0, 10.0})
    }
...
public savage_think(TankEnt)
{
    ...
    /*
    new Float: Asize[6]
    pev( TankEnt, pev_size, Asize)
    client_print(0, print_console, "Asize _ %f , %f , %f , %f , %f , %f ,", Asize[0], Asize[1], Asize[2], Asize[3], Asize[4], Asize[5])
    == Asize _ 400.000000 , 400.000000 , 81.000000 , 0.000000 , 0.000000 , 0.000000 ,
...
{ 
    if (!pev_valid(g_idTank[id])) return;
