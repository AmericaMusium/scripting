...
if(!is_valid_ent( iDog_id )) //Если NPC не существует
	...
	{	
		...
		if( fVelocity[2] < 40.0 && fDistance > 290.0) fVelocity[2] += 100.0
		
		new Float:fDistanceBlock
		fDistanceBlock = get_distance_f(fNOrigin, flastDogOrigin)
		
		// if(pev(iDog_id, pev_sequence) = 4 && BlockDist)  random+velocit
		// if(pev(iDog_id, pev_sequence) = 4 
		// client_print(0, print_chat, "fVel Z  =%f", fVelocity[2])
		// client_print(0, print_chat, "fVel x = %f  fVel y = %f", fVelocity[0], fVelocity[1])
		
		// fSpeed = 0.3 // fDisBlock 30.0
		if( fSpeed > 0.3 && fDistanceBlock < 30.0){ 
				
		fVelocity[0] += random_float(-400.0, 400.0)
		fVelocity[1] += random_float(-400.0, 400.0)
	...
	find_numberOfweaponbox(dog)
{
	...
	////// ПОКА ЦЕЛЬЮ ЯВЛЯЕТСЯ ПРЕДМЕТ, запускать поиск на Player 
	// если дистанция до ENEMY < 200 unitov , то Переключить цель на Врага. 
	/// Выполнять поиск цели двумя разными функциями ?? 
	// TARGET = weaponbox , teeth == 0 , Enemy close,  	 	// assign target to enemy 
	// TARGET = weaponbox , teeth == 0 , Enemy middle, 	 	//  assign target to enemy
	// TARGET = weaponbox , teeth == 0 , Enemy none			 //                          //  get weaponbox , teeth = 1 assigne target owner
	// TARGET = weaponbox , teeth = 1 , Enemy none,    		 //  assign target to OWNER //
	// TARGET = owner , teeth == 1/0 , Enemy none    		 //  assign target to owner or search weaponbox
	// TARGET = owner , teeth == 1/0 , Enemy close,    		 //  assign target to enemy , teeth = 0 ,
	// TARGET = owner , teeth == 1 , Enemy close,      		 //  assign target to enemy , teeth = 0 ,
	// TARGET = owner , teeth == 1 , Enemy none			//  assign target to enemy , teeth = 0 ,
	// TARGET = enemy , teeth == 0 , Enemy closer than TARGET	 //  assign target to NEW ENEMY , teeth = 0 
