...
public plugin_init()
{
 ...
 //   register_clcmd("say /scount", "Count")
    ...
    g_pCvarSavageCarHP = register_cvar("zp_savage_health", "2000.0")
...
public CreateSavage(id)
{
    static func_breakable_id
 
    if (!func_breakable_id)
        func_breakable_id = engfunc(EngFunc_AllocString, "info_target")
...
SpawnSavage(id)
{
    ...
    */
    ...
    engfunc(EngFunc_SetSize, SavEnt, {0.0, 0.0, 0.0}, {600.0, 600.0, 200.0})
    /*
    new Float:mins[3]
    mins[0] = -200.0
    mins[1] = -200.0
    ...
    maxs[1] = 200.0
    maxs[2] = 200.0
    set_pev(SavEnt, pev_mins, mins)
    set_pev(SavEnt, pev_maxs, maxs)
    set_pev(SavEnt, pev_mins, mins)
    */
    /*
    ...
    pev(SavEnt, pev_size, flSizeMin[0])
    pev(SavEnt, pev_size, flSizeMin[1])
    pev(SavEnt, pev_size, flSizeMin[2])
    // точно так же с min_size
    //set_pev(SavEnt, pev_mins, flSizeMin)
    flSizeMin[0] = -200.0
    flSizeMin[1] = -200.0
    flSizeMin[2] = 0.0
    set_pev(SavEnt, pev_mins, flSizeMin)
    */
    ...
    set_pev(SavEnt, pev_friction, 0.01)
    ...
    g_bIsSpawningCar[id] = false
    remove_preview(id)
    //new Float:ssiizzee[3]
    //;//pev(SavEnt, pev_size, ssiizzee[0] )
    //;server_print("%f %f %f ", ssiizzee[0], ssiizzee[1], ssiizzee[2])
