...
#define PLUGIN "DOD ThrowKnife Weaponbox"
#define VERSION "1.4"
...
ReTuneMovetype   // Back movetype after stack in wall
*/
...
public plugin_init() {
	...
	// RegisterHam(Ham_Touch ,"player","f_spade_attack2")
...
{	
	new bx_owner = entity_get_edict(ent, EV_ENT_owner )
...
{
	// if(set_pev(flyknifeid, pev_velocity,{60.0 , 50.0 , 50.0})
	...
	if(flyknifeid > 32 &&  touchid <= 32){ 
		...
		pev(flyknifeid,pev_velocity,NewVector)
		...
		*/
		...
		if(touchid == 0 && equal(status,"damager") && equal(classname,"fly_knife") ) {
			...
			set_pev(flyknifeid, pev_avelocity,{60.0 , 50.0 , 150.0})
			set_pev(flyknifeid, pev_velocity,{60.0 , 50.0 , 50.0})
			
			if(get_pcvar_num(cv_stackinwall))
			{
				
				set_pev(flyknifeid, pev_movetype, MOVETYPE_NONE)
				set_task(2.0, "ReTuneMovetype", flyknifeid)
			}
		...
		if( is_user_connected(touchid) && is_user_alive(touchid) && touchid != idAttacker && flyknifeid != idAttacker){
			...
			if(equal(status,"damager") &&  equal(classname,"fly_knife")) {
				...
				|| !is_user_alive(touchid)) {
					...
					*/
					...
					emit_sound(flyknifeid,CHAN_AUTO,g_ThrowHitHumanSound,0.4,ATTN_IDLE ,0,PITCH_NORM + 50)
					set_pev(flyknifeid, pev_classname, "weaponbox")
					set_pev(flyknifeid, pev_globalname,"nifebox")
					set_pev(flyknifeid,pev_avelocity,{60.0 , 50.0 , 150.0})
					set_pev(flyknifeid, pev_velocity,{60.0 , 50.0 , 50.0})
...
public ReTuneMovetype(flyknife){
	
	if(pev_valid(flyknife)){
		
		set_pev(flyknife,pev_movetype,MOVETYPE_TOSS)
		set_pev(flyknife,pev_avelocity,{60.0 , 50.0 , 150.0})
		set_pev(flyknife,pev_velocity,{0.0 , 0.0 , 0.0})
	}
}
