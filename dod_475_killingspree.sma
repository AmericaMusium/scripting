...
#define KS_MESSAGE "Fun time :D"	// Default message (use \n for 'New Line')
#define KS_WARNING "misc/killingspree.wav"				// Default soundbite
#define KS_WARNING1 "misc/ks_journey_dsb.wav"				// Default soundbite
#define KS_WARNING2 "misc/ks_tft_ebwtrw.wav"				// Default soundbite
#define KS_WARNING3 "misc/ks_tft_shout.wav"				// Default soundbite
#define KS_WARNING4 "misc/ks_erika.wav"				// Default soundbite
#define KS_WARNING5 "misc/ks_batotai.wav"				// Default soundbite
#define KS_WARNING6 "misc/ks_nabezvysot.wav"				// Default soundbite
#define KS_WARNINGUS "ambience/uswin.wav"
#define KS_WARNINGAX "ambience/germanwin.wav"
#define KS_WARNINGBR "ambience/britwin.wav"
...
public plugin_precache()
{
	...
	precache_sound(KS_WARNING4)	// Notify soundbite warning
	precache_sound(KS_WARNING5)	// Notify soundbite warning
	precache_sound(KS_WARNING6)	// Notify soundbite warning

	precache_sound(KS_WARNINGUS)	// Notify soundbite warning
	precache_sound(KS_WARNINGAX)	// Notify soundbite warning
	precache_sound(KS_WARNINGBR)	// Notify soundbite warning
...
public plugin_init()
{
	...
	g_respawn = register_cvar("dod_killingspree_respawn", "1")		// Allow respawns during killingspree?
...
public HAM_score_ent_USE_post(ent)
{
	switch (g_control) {
		case KS_PREP: {
			...
			set_task(0.1, "do_stop_sound_for_all")
			set_task(0.4, "ks_notify")
			g_control = KS_KILL
		}
...
public ks_notify()
{
	if (get_pcvar_num(g_notify)) {
		...
		new random_message = random_num(1,6)
		switch (g_team)
				{
					case 1: random_message =  random_num(1,3)
					case 2: random_message = random_num(4,6)
					default: random_message = 1
				}
		
		switch (random_message)
		{
			case 1: format(ksmess, 127, "spk %s", KS_WARNINGUS)
			case 2: format(ksmess, 127, "spk %s", KS_WARNINGBR)
			case 3: format(ksmess, 127, "spk %s", KS_WARNING6) //format(ksmess, 127, "spk %s", KS_WARNING3)
			case 4: format(ksmess, 127, "spk %s", KS_WARNING4) //format(ksmess, 127, "spk %s", KS_WARNING3)
			case 5: format(ksmess, 127, "spk %s", KS_WARNING5) //format(ksmess, 127, "spk %s", KS_WARNING3)
			case 6: format(ksmess, 127, "spk %s", KS_WARNINGAX) //format(ksmess, 127, "spk %s", KS_WARNING3)
		}
		//emit_sound(0, CHAN_AUTO, ksmess, 0.3, ATTN_NORM, 0, PITCH_NORM);
		client_cmd(0, ksmess)
	}
...
public fw_emit_sound(id,channel,const sound[])
{
    ...
    {	
		client_print(0, print_chat,"bonenum  5555555555555555555")	
