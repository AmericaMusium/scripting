...
new session_id_to_registerlist[50]
...
{  
    ...
    {   
        ...
        {   
            ...
            server_print("###REGISTRATION LIST## List ID: %d , Steam: %s , Teamkills in this session: %d", i ,stID, g_cache_stID[i][tkills])
            // найти порядковый номер в ресстре, и
            //  на основе порядкового номера посчитать teamkills
            if( g_cache_stID[i][tkills] >= 10)
            {   
                server_print("###REGISTRATION LIST## BANNED List ID: %d , Steam: %s , Teamkills in this session: %d", i ,stID, g_cache_stID[i][tkills])
                // ban kick
            }
            else
            {   
                server_print("###REGISTRATION LIST## HIS KILLS is %d of 10!", g_cache_stID[i][tkills])
        ...
        else if(!equal(stID, g_cache_stID[i][STID]))
        {
            ...
            server_print("###REGISTRATION LIST## REGISTERED NEW PLAYER to LIST %d : Steam: %s", i ,stID)
            return PLUGIN_HANDLED_MAIN 
        }

        else return PLUGIN_HANDLED_MAIN 
    }

    // get_user_ip(player, stID, 32)
...
public client_death(killer,victim,wpnindex,hitplace,TK)
{
	...
	if (get_user_team(killer) == get_user_team(victim) && killer!=victim )
    {
        ...
        if( g_cache_stID[session_id_to_registerlist[killer]][tkills] >= 10)
        {
        ...
        server_print("###REGISTRATION LIST## TEAMK KILL :%d : %d", killer ,victim)
...
TKPunish(killer, victim)
{
	...
	g_TKcount[killer] = g_cache_stID[session_id_to_registerlist[killer]][tkills]
