...
public plugin_init() 
{
    ...
    RegisterHam(Ham_Weapon_SecondaryAttack, "weapon_mg34", "hook_PlayerPreThink", true);
...
public hook_PlayerPreThink( idx_weapon ) 
{   
    
    new idx_owner = get_pdata_cbase(idx_weapon, m_pPlayer, linux_diff_weapon);
    client_print(idx_owner,print_chat,"secondary");
	if( !is_user_alive( idx_owner ) || !get_pcvar_num( g_cvarEnabled ) || is_user_bot( idx_owner ) ) return PLUGIN_CONTINUE;
	
	if( dod_get_deploystate( idx_owner ) < 2 && !touching_sandbag( idx_owner ) )
	{
		dod_set_deploystate( idx_owner, get_deploy_ability( idx_owner ) );
	}
