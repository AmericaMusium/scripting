...
//	dod_smoke_mp40 "1"
//	dod_smoke_mp44 "1"
//	dod_smoke_scharfschutze "1"
//	dod_smoke_fg42 "1"
//	dod_smoke_scoped_fg42 "1"
//	dod_smoke_mg34 "1"
//	dod_smoke_mg42 "1"
//	dod_smoke_panzerjager "1"
//	dod_smoke_enfield "1"
//	dod_smoke_sten "1"
//	dod_smoke_marksman "1"
//	dod_smoke_bren "1"
//	dod_smoke_piat "1"
//
//////////////////////////////////////////////////////////////////////////////////
...
//	SETTING_SIZEMIN		30	//Minimum smoke sprite size
//	SETTING_SIZEMAX		80	//Maximum smoke sprite size
//	SETTING_SPRITES		6	//Number of smoke sprites created each second
//	SETTING_DURATION	25.0	//How many seconds grenades will give off smoke
...
//
//	SETTING_MAXSMOKE	20	//Sets the max number of smoke grenades allowed at once 
...
#define SETTING_SIZEMAX		50		
#define SETTING_SPRITES		9
...
#define SETTING_CHOKE		0
#define SETTING_CHOKERADIUS	210.0
#define SETTING_CHOKECHANCE	0

#define SETTING_DAMAGE		0
#define SETTING_DAMAGEAMT	2
#define SETTING_DAMAGECHANCE	0

#define SETTING_DAMAGEHIT	0
...
public plugin_init()
{
	...
	p_garand = register_cvar("dod_smoke_garand","0")
	p_carbine = register_cvar("dod_smoke_carbine","1")
	p_thompson = register_cvar("dod_smoke_thompson","2")
	p_grease = register_cvar("dod_smoke_grease","2")
	p_sniper = register_cvar("dod_smoke_sniper","0")
	p_bar = register_cvar("dod_smoke_bar","2") 
	p_30cal = register_cvar("dod_smoke_30cal","0")
	p_bazooka = register_cvar("dod_smoke_bazooka","0")
	p_kar = register_cvar("dod_smoke_kar","0")
	p_k43 = register_cvar("dod_smoke_k43","1")
	p_mp40 = register_cvar("dod_smoke_mp40","2")
	p_mp44 = register_cvar("dod_smoke_mp44","2")
	p_scharf = register_cvar("dod_smoke_scharfschutze","0")
	p_fg42 = register_cvar("dod_smoke_fg42","1")
	p_scopedfg = register_cvar("dod_smoke_scoped_fg42","1")
	p_mg34 = register_cvar("dod_smoke_mg34","0")
	p_mg42 = register_cvar("dod_smoke_mg42","0")
	p_panzer = register_cvar("dod_smoke_panzerjager","0")
	p_enfield = register_cvar("dod_smoke_enfield","1")
	p_sten = register_cvar("dod_smoke_sten","2")
	p_marksman = register_cvar("dod_smoke_marksman","0")
	p_bren = register_cvar("dod_smoke_bren","0")
	p_piat = register_cvar("dod_smoke_piat","0")
...
public throwsmoke_real(id) 
{
	...
	emit_sound(id,CHAN_BODY,fileNames[throw],0.4,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))
...
public say_smoke(id)
{
	...
	// 0722 emit_sound(id,CHAN_VOICE,fileNames[sound_to_play],0.8,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))
		
	// 0722 client_cmd(id,"say_team Smoke Em!")
	...
	if(equali(classname,"dod_smoke") && !g_round_restart)
		...
		if(gametime < pev(grenid,pev_fuser1))
			...
			if(pev(grenid,pev_iuser1))
				{
				emit_sound(grenid,CHAN_WEAPON,fileNames[explode_smoke],0.3,ATTN_NORM,0,PITCH_NORM-70)
	...
	while((ent = engfunc(EngFunc_FindEntityInSphere,ent,GrenOrigin,SETTING_CHOKERADIUS)) != 0)
		...
		if(equali(classname,"player") && is_user_alive(ent) && !get_user_godmode(ent))
			...
			if(random_num(1,SETTING_CHOKECHANCE) == 1)
				// 0722 emit_sound(ent,CHAN_VOICE,fileNames[random_num(cough1, cough2)],0.3,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))		
			...
			if(random_num(1,SETTING_DAMAGECHANCE) == 1)
				...
				if(attacker_team != victim_team || (attacker_team == victim_team && get_pcvar_num(p_friendlyfire)))
					...
					if(current_health > smoke_damage)
						{							
						// emit_sound(ent,CHAN_VOICE,fileNames[random_num(cough1,grunt)],0.3,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))
						// 0722 emit_sound(ent,CHAN_BODY,fileNames[grunt],1.0,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))
	...
	if(equali(classname,"dod_smoke"))
		...
		if(!equali(obj_classname,"player"))
			...
			if((!friendlyfire || (friendlyfire && get_pcvar_num(p_friendlyfire))) && !get_user_godmode(object))
				...
				// 0722 emit_sound(object,CHAN_VOICE,fileNames[grunt],1.0,ATTN_NORM,0,PITCH_NORM + random_num(-30, -20))
				}
			else
				{				
				switch(random_num(0,2))
...
public round_message() 
{
	if(read_data(1) == 1)
		...
		while((grenid = engfunc(EngFunc_FindEntityByString, -1,"classname","dod_smoke")) > 0)
			{
			emit_sound(grenid,CHAN_WEAPON,fileNames[explode_smoke],0.3,ATTN_NORM,SND_STOP,PITCH_NORM + random_num(-30, -20))
