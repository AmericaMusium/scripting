...
SpawnSavage(id)
{
    ...
    set_pev(SavEnt, pev_solid, SOLID_BBOX)
    set_pev(SavEnt, pev_movetype, MOVETYPE_PUSHSTEP )
    set_pev(SavEnt, pev_friction, 0.5)
    set_pev(SavEnt, pev_gravity, 1.0)
    set_pev(SavEnt, pev_takedamage, 1.0)
    engfunc(EngFunc_SetSize, SavEnt, Float:{-100.0, -100.0, -80.0},{100.0, 100.0, 20.0})
   ...
   //  get_user_name(id, szName, charsmax(szName))
    ...
    client_print(0, print_chat,"Spawned Race Car")
...
public client_PreThink(id)
{
    ...
    if (pev(id, pev_button) & IN_JUMP && g_bHasSpawned[id] && !g_bIsSvgMenuOpened[id] && g_idTank[id])
    ...
    pev(id, pev_v_angle, fAngA)


    client_print(id, print_center, "Yaw0:%i  Yaw1: %i", floatround(fAngA[0]) , floatround(fAngA[1]) )
...
public savage_think(SavEnt)
{
    ...
    // Input handling
    new buttonpress = 0
    if (g_idCamera[SavID] && g_idTank[SavID])
    ...
    // set_pev(SavID, pev_viewmodel2, WEAPON_MODEL_VIEW);
    ...
    {
        emit_sound(SavEnt, CHAN_VOICE, RC_SOUND_ENGINE, VOL_NORM, ATTN_NORM, 0, PITCH_NORM )
        fSavages[SavID][eNextSoundTime] = halflife_time() + 0.5
    }
    else if(fSavages[SavID][eNextPitchTime] < halflife_time())
    {
        emit_sound(SavEnt, CHAN_VOICE, RC_SOUND_ENGINE , VOL_NORM, ATTN_NORM, SND_CHANGE_PITCH, PITCH_NORM )
...
public camera_think(CamEnt)
{
    ...
    pev(g_idTank[SavID], pev_origin, TargetOrigin)
...
{ 
	...
	new iOrigin[3], Float:fOrigin[3], Float:Vel[3], Float:Angles[3], Float:RAngles[3]
    set_task(0.1, "rocket_retune", rocketindex)
    /*
    TANK ENT HAT THIS :
    set_pev(SavEnt, pev_angles, vAngles)
    set_pev(SavEnt, pev_avelocity, vAVelocity)
    ...
    */
    ...
    // ориентация модели
    // pev(g_idTank[id], pev_angles, Angles)
    // set_pev(rocket, pev_angles, Angles)
    pev(g_idTank[id], pev_velocity, Vel)
    ...
    set_pev(rocketindex, pev_movetype, MOVETYPE_NONE)
    // MOVETYPE_TOSS     
    client_print(0, print_chat, "Rocket id %d" , rocketindex)
    // console_cmd(id,"say /parser;")
