...
#define this_is_throwed_knife(%1) entity_set_int(%1,EV_INT_iuser3, YES_THIS_IS_THROWEDKNIFE)
...
public ThrowedKnife_Throw(idx_player)
{
	...
	this_is_throwed_knife(idx_throwed_knife);
...
{	
	if(!pev_valid(idx_throwed_knife) || !pev_valid(idx_object))	return HAM_IGNORED;
	new g_couner = 0;
	switch(is_this_throwed_knife(idx_throwed_knife))
	{
		...
		{	
			...
			if(knife_owner == idx_object && last_touch_mode == TOUCH_DAMAGE) return HAM_IGNORED;
			...
			switch (last_touch_mode)
			{
				case TOUCH_DAMAGE:
				{
					if(is_user_alive(idx_object) && knife_owner != idx_object)
					{	
						server_print("TOUCH_DAMAGE go go og %d", g_couner);
						g_couner++;
				...
				{	
					server_print("TOUCH_WEAPONBOX o go og %d", g_couner);
					g_couner++;
			...
			// это нож, коснулся хуйнюшки Валидной 

			// ANY TOUCH
            if(is_user_alive(idx_object)) return HAM_IGNORED;
			...
			/*
			if (pev(idx_throwed_knife, pev_flags) & FL_ONGROUND ) // Сущность находится на частично твердой поверхности
			if (pev(idx_throwed_knife, pev_flags) & FL_PARTIALGROUND ) // FL_PARTIALGROUND == cущность находится на частично твердой поверхности
			*/

			//// Destoroy Glass::idx_object == func_breakable 
			new clT[32];
			...
			if(equal(clT,"func_breakable")) 
			{
				...
				if(!tgn[0])
				{
					ExecuteHam(Ham_TakeDamage, idx_object, 1, 1, 100.0, DMG_BULLET);
...
{
	set_rendering(idx_throwed_knife, kRenderFxGlowShell, 255, 150, 50, kRenderNormal, 16);
}
...
{	
	...
	{	
		...
		{
			emit_sound(idx_throwed_knife, CHAN_AUTO, g_ThrowSound, 0.3, ATTN_NORM, 0, PITCH_HIGH);
