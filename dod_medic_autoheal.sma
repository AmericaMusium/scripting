...
new g_medic_sound
new g_sounds[3][] = {"ambience/tree.wav", "ambience/tree.wav", "ambience/tree.wav"}
...
{
	precache_sound("ambience/tree.wav")
}
...
public plugin_init()
{
	...
	register_clcmd("say /medic", "cmdMedic", 0, "Call for a Medic")
	register_clcmd("medic", "cmdMedic", 0, "Call for a Medic")
	...
	if (get_pcvar_num(g_medic_ctrl) == 1 || get_pcvar_num(g_medic_ctrl) == 3) {
		if (g_medic_calls[id]) {
			...
			if (get_pcvar_num(g_medic_sound)) {
				...
				emit_sound(id, CHAN_ITEM, g_sounds[myteam], 0.9, ATTN_NORM, 0, 20)
			}
			if (pev(id, pev_health) >= get_pcvar_num(g_medic_maxhp)) {
				// client_print(id, print_chat, "You are healty!")
