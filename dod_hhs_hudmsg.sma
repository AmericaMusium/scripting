...
public HUD_print_color_mix()
{
     ...
     {    
          if(g_fadeinfx == 1)
          {
               new floag: fin = g_HUD_SCRL_BAR[str][fadeintime]
               fin = float((MAX_LINES-1)-str)*0.05
               g_HUD_SCRL_BAR[str][fadeintime] = fin
          }
        
          set_dhudmessage(g_HUD_SCRL_BAR[str][r],g_HUD_SCRL_BAR[str][g],g_HUD_SCRL_BAR[str][b], HUD_X , (HUD_Y + (HUD_S * float(str))) , g_HUD_SCRL_BAR[str][effects], g_HUD_SCRL_BAR[str][fxtime], g_HUD_SCRL_BAR[str][holdtime], g_HUD_SCRL_BAR[str][fadeintime], g_HUD_SCRL_BAR[str][fadeouttime], true)
...
public spam()
{
    ...
    for (new i = 0; i < 12; i++)
    {
          ...
          set_dhudmessage(g_HUD_SCRL_BAR[i][r], g_HUD_SCRL_BAR[i][g], g_HUD_SCRL_BAR[i][b], random_float(0.1,0.9),random_float(0.1,0.9), g_HUD_SCRL_BAR[i][effects], g_HUD_SCRL_BAR[i][fxtime], g_HUD_SCRL_BAR[i][holdtime], g_HUD_SCRL_BAR[i][fadeintime], g_HUD_SCRL_BAR[i][fadeouttime], true)
