...
InitializeReplacementType(const Type)
{
	switch(Type)
	{
		case CompleteReplacement:
		{
			...
			formatex(MapFilePath, charsmax(MapFilePath), "%s/%s-%s.ini", ConfigsDirPath, CompleteReplaceFileName, MapName_def)
		...
		case DynamicReplacement:
		{
			formatex(MapFilePath, charsmax(MapFilePath), "%s/%s-%s.ini", ConfigsDirPath, DynamicReplaceFileName, MapName_def)
