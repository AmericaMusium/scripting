...
public plugin_init() {
	...
	p_max_mortar = register_cvar("swm_max_piat","5")
...
public func_gun_menu(id) {
				...
				if(get_pcvar_num(p_autospawn_weapon) == 1)
	{
		...
		{
			menu_additem(gun_menu, "Enable Gun Save \r[DISABLED]", "24", 0)
			client_cmd(id, "/guns")
		}
		else
		{
			menu_additem(gun_menu, "Disable Gun Save \y[ENABLED]", "24", 0)
			client_cmd(id, "/guns")
		}
	}
	...
	menu_additem(gun_menu,"Mortar","25",0)
				if(get_pcvar_num(p_autospawn_weapon) == 1)
	{
		if (client_gunsave[id] == 0)
		{
