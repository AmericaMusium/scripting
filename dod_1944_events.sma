...
new cv_enable_chat = 0;     // включить чат событий
new cv_enable_fullchat = 0; // включить чат всех событий без ограничений по количествву килов и т.д.


new cv_enable_timer = 0;    // показывать время раунда в окне HUD_CHAT
new cv_write_stats = 0;     // записывать ли статистику ?
new cv_enable_top = 0;      // запускает функцию сортировки по топу. 
new cv_enable_dog_message = 0;  // Вкл\Выкл уведомления с собачкой
...
{   
    ...
    // В этой функции TK не регистриурется при убийстве с лопатки и со всех базук
    // по этому я снова перешёл на отлолв on_DeathMsg
    ...
    if(!is_user_connected(idx_killer)) return; // Добавить
    if(!is_user_connected(idx_victim)) return; // Добавить
    if(id_weapon < 0 || id_weapon > 42) return; // Добавить

    if (!idx_killer || idx_killer > MAX_PLAYERS) return;
    if (!idx_victim || idx_victim > MAX_PLAYERS) return;
    // Cброс Жертве
    if (cv_write_stats)
    {
...
{       
    ...
    if(idx_killer < 0 || idx_killer > MAX_PLAYERS) return; // Добавить
    if(idx_victim < 1 || idx_victim > MAX_PLAYERS) return; // Добавить
    if (!idx_killer || !idx_victim) return; // Пропускаем если killer/victim - мир
    if (idx_killer > MAX_PLAYERS || idx_victim > MAX_PLAYERS) return;
    if (!is_user_connected(idx_killer)) return;
    //////////////////////////// Weapons !
    switch (id_weapon)
    {   
        ///////////////////////  BazookaKills
...
{   
    ...
    if (!pev_valid(idx_player)) return;
    if (idx_player < 1 || idx_player > MAX_PLAYERS) return; // Проверка на валидность
    if (!is_user_connected(idx_player)) return;
    ...
    switch (ev_type)
    {
        case DamageDealt:
        {   
            format(ExportMessage, 191, "%s %s Нанёс урон", ExportMessage, sz_UserName);
            if(cv_write_stats || cv_enable_top)
            {
                if(g_[life][DamageDealt][idx_player] > 1000)
                {   
                    format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][DamageDealt][idx_player]);
                    bonus_impact++;
                }
                if(g_[round][DamageDealt][idx_player] > 5000)
                {
                    format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][DamageDealt][idx_player]);
                    bonus_impact++;
                }
                if(g_[match][DamageDealt][idx_player] > 10000)
                {
                    format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][DamageDealt][idx_player]);
                    bonus_impact++;
                }
            }
            if(bonus_impact)
        ...
        case DamageTaken:
        {   
            format(ExportMessage, 191, "%s %s получил урон", ExportMessage, sz_UserName);
            if(cv_write_stats || cv_enable_top)
            {
                if(g_[life][DamageTaken][idx_player] > 200)
                {
                    format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][DamageTaken][idx_player]);
                    bonus_impact++;
                }
                if(g_[round][DamageTaken][idx_player] > 5000)
                {
                    format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][DamageTaken][idx_player]);
                    bonus_impact++;
                }
                if(g_[match][DamageTaken][idx_player] > 10000)
                {
                    format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][DamageTaken][idx_player]);
                    bonus_impact++;
                }
            }
            if(bonus_impact)
        ...
        case TotalKills: 
        {   
            format(ExportMessage, 191, "%s %s убил ", ExportMessage, sz_UserName);

            if(g_[life][TotalKills][idx_player] > 2)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][TotalKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][TotalKills][idx_player] > 4)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][TotalKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][TotalKills][idx_player] > 8)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][TotalKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case GrenadeKills:
        {
            format(ExportMessage, 191, "%s %s убил гранатой", ExportMessage, sz_UserName);
            if(g_[life][GrenadeKills][idx_player] > 5)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][GrenadeKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][GrenadeKills][idx_player] > 20)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][GrenadeKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][GrenadeKills][idx_player] > 40)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][GrenadeKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case ButtKills:
        {   
            format(ExportMessage, 191, "%s %s убил прикладом", ExportMessage, sz_UserName);
            if(g_[life][ButtKills][idx_player] > 3)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][ButtKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][ButtKills][idx_player] > 15)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][ButtKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][ButtKills][idx_player] > 30)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][ButtKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case KnifeKills:
        {   
            format(ExportMessage, 191, "%s %s убил ножом", ExportMessage, sz_UserName);
            if(g_[life][KnifeKills][idx_player] > 3)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][KnifeKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][KnifeKills][idx_player] > 15)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][KnifeKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][KnifeKills][idx_player] > 30)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][KnifeKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        {   
            // Здесь ебаная ошибка ! 
            format(ExportMessage, 191, "%s %s убил своего", ExportMessage, sz_UserName);
            if(g_[life][TeamKills][idx_player] > 1)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][TeamKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][TeamKills][idx_player] > 10)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][TeamKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][TeamKills][idx_player] > 20)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][TeamKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case Suicides:
        {   
            format(ExportMessage, 191, "%s %s умер сам", ExportMessage, sz_UserName);
            if(g_[round][Suicides][idx_player] > 10)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][Suicides][idx_player]);
                bonus_impact++;
            }
            if(g_[match][Suicides][idx_player] > 20)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][Suicides][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        {
            format(ExportMessage, 191, "%s %s захватил флаг", ExportMessage, sz_UserName);
            if(g_[life][PlayerFlagCaptures][idx_player] > 5)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][PlayerFlagCaptures][idx_player]);
                bonus_impact++;
            }
            if(g_[round][PlayerFlagCaptures][idx_player] > 20)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][PlayerFlagCaptures][idx_player]);
                bonus_impact++;
            }
            if(g_[match][PlayerFlagCaptures][idx_player] > 100)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][PlayerFlagCaptures][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case BazookaKills:
        {
            format(ExportMessage, 191, "%s %s убил ракетой", ExportMessage, sz_UserName);
            if(g_[life][BazookaKills][idx_player] > 5)
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][BazookaKills][idx_player]);
                bonus_impact++;
            }
            if(g_[round][BazookaKills][idx_player] > 20)
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][BazookaKills][idx_player]);
                bonus_impact++;
            }
            if(g_[match][BazookaKills][idx_player] > 100)
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][BazookaKills][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        case HeadShots:
        {
            format(ExportMessage, 191, "%s %s убил, пробив голову ", ExportMessage, sz_UserName);
            if(g_[life][HeadShots][idx_player] > 2) // 5
            {
                format(ExportMessage, 191, "%s %d за жизнь", ExportMessage, g_[life][HeadShots][idx_player]);
                bonus_impact++;
            }
            if(g_[round][HeadShots][idx_player] > 4) // 
            {
                format(ExportMessage, 191, "%s %d за раунд", ExportMessage, g_[round][HeadShots][idx_player]);
                bonus_impact++;
            }
            if(g_[match][HeadShots][idx_player] > 6) //
            {
                format(ExportMessage, 191, "%s %d за матч", ExportMessage, g_[match][HeadShots][idx_player]);
                bonus_impact++;
            }
            if(bonus_impact)
            {
        ...
        {
            format(ExportMessage, 191, "%s %s напал первым!", ExportMessage, sz_UserName);
            HW_Message_Print(ExportMessage, idx_team);
            return;
        }
        case EVENT_TYPE + EV_ROUNDEND:
        {
            format(ExportMessage, 191, "%s Раунд закончился", ExportMessage);
            switch (idx_player)
            {
                case ALLIES: format(ExportMessage, 191, "%s победой Allies", ExportMessage);
                case AXIS: format(ExportMessage, 191, "%s победой Axis", ExportMessage);
                default: format(ExportMessage, 191, "%s напрасно и безрезультатно", ExportMessage);
            }
        ...
        default: 
        {
            format(ExportMessage, 191, "%s %s default case!  %d !", ExportMessage, sz_UserName, ev_type);
            HW_Message_Print(ExportMessage, basic);
        }
    }
}
