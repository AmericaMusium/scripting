...
dod_tw_damage 160			// damage on hit
dod_tw_speed 1000		// throw speed
...
new const g_ThrowSound[] = "weapons/knife_slash2.wav"
new const g_ThrowFlySound[] = "player/pl_grate4.wav"
...
public plugin_init()
{
	...
	p_wpn_damage = register_cvar("dod_tw_damage", "160")
	p_wpn_speed = register_cvar("dod_tw_speed", "1000.0")
	...
	register_clcmd("+attack2","throw_wpn")	
...
public SetModel(iEnt)
{
	...
	if(is_user_alive(id) && !is_wpn_heavy(id) && g_can_throw[id])
	{
		...
		fVelocity[0] = f2Velocity[0] + random_float(5.0,-5.0)   //
		fVelocity[1] = f2Velocity[1] + random_float(5.0,-5.0)
		fVelocity[2] = f2Velocity[2] + random_float(71.0, 110.0) // all correct
		
		
		/// rotation 
		new Float:TumbleVector[3]			
		TumbleVector[0] = 600.0 // = 1320.0  // Wheel
		TumbleVector[1] // = random_float(800.0,100.0)  // TEA
		TumbleVector[2] // 400.0 // = random_float(2.0,0.0) //  HOURS
		
		/*
		/// angle 
		
		new Float:fmodAngle[3]
		fmodAngle[0] = 55.0 // = 100.0// pitch
		fmodAngle[1] = 90.0// = 100.0 //  100.0 // yaw +++
		fmodAngle[2] = 90.0 //  = 100.0 // roll
		set_pev(iEnt, pev_angles, fmodAngle)
		*/
		///////////////// BEGINS //////////////// 
		new Float:f5Aim[3], Float:f5Angles[3], Float:f5Origin[3], Float:f52Angles[3]
		velocity_by_aim(id, 400, f5Aim)
		vector_to_angle(f5Aim, f5Angles)
		...
		f52Angles[0] = f5Angles[0] + 55.0
		f52Angles[1] = f5Angles[1] // + 90.0
		f52Angles[2] = f5Angles[2] + 90.0
		// Set the origin and view
	
		// set_pev(iEnt, pev_origin, f5Origin)
		set_pev(iEnt, pev_angles, f52Angles)
		////////////////////  ENDS //////////
		...
		// entity_set_int(iEnt, EV_INT_movetype, MOVETYPE_FLY)   // works!	
		g_can_throw[id]=false
		set_task(get_pcvar_float(p_wpn_time_alive),"BackToNormal",iEnt)
		emit_sound(id,CHAN_BODY,g_ThrowSound,0.9,ATTN_NORM,0,PITCH_NORM+random_num(-10,30))
		emit_sound(iEnt,CHAN_AUTO,g_ThrowFlySound,1.0,ATTN_NORM ,0,PITCH_NORM+random_num(30,90))
...
{			
	if(get_pcvar_num(p_on) && is_user_connected(id) && is_user_alive(id))
	{
		...
		custom_weapon_dmg(wpn_id,iAttacker,id,get_pcvar_num(p_wpn_damage),hitplace)
				...
				new iknifeOrigin[3]
				get_user_origin(id,iknifeOrigin,0)
				message_begin(MSG_BROADCAST,SVC_TEMPENTITY)
				write_byte(TE_SPARKS)
				write_coord(iknifeOrigin[0])
				write_coord(iknifeOrigin[1])
				write_coord(iknifeOrigin[2])
				message_end()
	...
	{
		//it_sound(iEnt,CHAN_AUTO,g_ThrowHitwallSound,0.4,ATTN_NORM,0,200)		
	}
	/*
				new iknifeOrigin[3]
				get_user_origin(id,iknifeOrigin,0)
