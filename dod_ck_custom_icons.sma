...
new hud_flag_icon1, hud_flag_icon2, hud_flag_icon3
...
public plugin_init()
{
	register_plugin("DOD CK custom","0.0","America")
...
{
server_print("Non-Post")
server_print("AlertMessage %i", at_type)
server_print("AlertMessage %s", message)
}

public fn_alertmessage_post(at_type, const msg[])
{
server_print("Post")
server_print("AlertMessage_Post %i", at_type)
server_print("AlertMessage_Post %s", msg)
}

public search_free_sprite_index()
{
   
    new idx_cpoint
	while((idx_cpoint = engfunc(EngFunc_FindEntityByString, idx_cpoint, "classname", "dod_control_point")) != 0)
	{   
        set_pdata_int(idx_cpoint,m_iNeutralIcon, target_idx, linux_diff_entity)  // 
        set_pdata_int(idx_cpoint,m_iAlliesIcon, target_idx+1, linux_diff_entity) // 
        set_pdata_int(idx_cpoint,m_iAxisIcon, target_idx+2, linux_diff_entity)  //
    }
