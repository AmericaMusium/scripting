...
new const szFilePath[32] = "addons/amxmodx/configs/";
...
new idx_player_saved_choice[33][32]
new g_mode
...
{   
    ...
    if(is_file_exists)
    {
        ...
        {
            while(fgets(file_handle, line, sizeof(line)) != 1)
            {
                line_num++;
                ...
                {   
                    ...
                    idx_player_line[idx_player] = line_num
                    server_print("[CUSTOM EQIPMENTS] VIP %s CONNECTED | file line = %d" , current_steam_id,line_num )
                    break;
...
// MENU CREATOR 
 ...
 public Menu_Button_pressed( idx_player, menu, item )
 {
    ...
    switch (g_mode)
    {   
        case 1: give_weapon_to_player_classical(idx_player , m_Name )
        case 2: give_weapon_to_player_additional(idx_player , m_Name )
    }
    menu_destroy( menu );
    return PLUGIN_CONTINUE;
 }
...
public give_weapon_to_player_classical(idx_player, const wname[])
{
	...
	{   
        new temp_cur_weapon_ent_id = get_pdata_cbase(idx_player, m_rifleItem) // to frop pistol use m_pistolItem
        if (temp_cur_weapon_ent_id != -1)
        {
            new temp_class_name[32]
            new Float:temp_vuser1[3]
