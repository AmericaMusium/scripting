...
public plugin_init()
{
	...
	p_wpn_speed = register_cvar("dod_tw_speed", "10.0")
...
public SetModel(iEnt)
{
	...
	if(is_user_alive(id) && !is_wpn_heavy(id) && g_can_throw[id])
	{
		...
		fVelocity[2] = f2Velocity[2] + random_float(11.0,10.0) // all correct
		/// rotation 
		new Float:TumbleVector[3]			
		TumbleVector[0] // = 400.0 // = 1320.0  // Wheel
		TumbleVector[1] = 400.0 // = random_float(800.0,100.0)  // TEA
...
{			
	...
	{
		emit_sound(iEnt,CHAN_AUTO,g_ThrowHitwallSound,0.4,ATTN_NORM,0,210)		
	}
	/*
				new iknifeOrigin[3]
				get_user_origin(id,iknifeOrigin,0)
