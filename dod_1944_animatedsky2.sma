...
new const g_Weather_Base_Model[][] = 
{
...
public plugin_precache()
{
	...
	*/
	...
	what = engfunc(EngFunc_PrecacheModel, g_Weather_Base_Model[0]);
	engfunc(EngFunc_PrecacheModel, g_Weather_Base_Model[1]);
	server_print(" PRECHACHEDDDDDDDDDDDDDDDD = %d %s", what,  g_Weather_Base_Model[0]);
}
...
public Sky_Create()
{
	...
	// set_pev(iEntity, pev_origin, {0.0, 0.0, -50.0});
...
{	
	...
	if (!is_enable_time)
	{
		...
		set_pev(g_Sky_Clouds, pev_skin, g_iSkin);
...
public Update_Skin()
{
    ...
    set_pev(g_Sky_Clouds, pev_skin,  g_stepsskin-g_current_skin_step);
}
...
{	
	...
	set_pev(iEntity, pev_angles, {0.0, 70.0, 0.0});  // ГАПРАВЛЕНИЕ ВРАЩЕНИЯ ШАРИКА {0.0, 0.0, -90.0});
	/// можно сделать цикл поворотов англес ))) тоже очень прокачает красоту)
	// set_pev(iEntity, pev_origin, {0.0, 0.0, 50.0});


	engfunc(EngFunc_SetModel, iEntity, g_Weather_Base_Model[0]);
	
	engfunc(EngFunc_SetSize, iEntity, Float:{-50000.0, -50000.0, -50000.0}, Float:{50000.0, 50000.0, 50000.0});

	
	set_pev(iEntity, pev_rendermode, kRenderNormal); // Render alpha
	/*
	kRenderNormal = заливает всё без вычитания маски + прозрачность не регулируется вообще
	kRenderTransAdd src*a+dest = второй слой сработает как маска , для отображения
	*/
	set_pev(iEntity, pev_renderamt, 255.0);
	set_pev(iEntity, pev_renderfx, kRenderFxNone); // kRenderFxFadeFast 2 

	Render_Fx_clouds();
	server_print("aaaaaaaaaaaaaaaaaaaaaaaaa Base sky created %d", g_Sky_Base);
...
public Render_Fx_clouds()
{	
	
	set_pev(g_Sky_Clouds, pev_rendermode, kRenderTransTexture); // Render alpha
	set_pev(g_Sky_Clouds, pev_renderamt, 35.0); // 100 наверно крайнее значение
	set_pev(g_Sky_Clouds, pev_renderfx, kRenderFxNone); // kRenderFxFadeFast 2 
...
{	
	...
	set_pev(iEntity, pev_solid, SOLID_NOT);
	set_pev(iEntity, pev_sequence, 0);
	set_pev(iEntity, pev_framerate, 1.8); 
	set_pev(iEntity, pev_angles, {0.0, -70.0, 0.0});  // ГАПРАВЛЕНИЕ ВРАЩЕНИЯ ШАРИКА {0.0, 0.0, -90.0});
	/// можно сделать цикл поворотов англес ))) тоже очень прокачает красоту)
	// set_pev(iEntity, pev_origin, {0.0, 0.0, 50.0});


	engfunc(EngFunc_SetModel, iEntity, g_Weather_Base_Model[1]);
	
	engfunc(EngFunc_SetSize, iEntity, Float:{-50000.0, -50000.0, -50000.0}, Float:{50000.0, 50000.0, 50000.0});

	
	set_pev(iEntity, pev_rendermode, kRenderTransAdd); // Render alpha
	set_pev(iEntity, pev_renderamt, 255.0); // 100 наверно крайнее значение
	set_pev(iEntity, pev_renderfx, kRenderFxNone); // kRenderFxFadeFast 2 

	server_print(" Sky_Create_Sun Sky_Create_Sun Sky_Create_Sun Sky_Create_SunSky_Create_Sun");
