...
if(!is_user_bot(id))
    {
        ...
        {   
            ...
            {   
                ...
                server_print("__________________ List ID: %d , Steam: %s , Teamkills in this session: %d", i ,stID, g_cache_stID[i][tkills])
                // найти порядковый номер в ресстре, и
                //  на основе порядкового номера посчитать teamkills
                if( g_cache_stID[i][tkills] >= 10)
                {   
                    server_print("__________________ LIST## BANNED List ID: %d , Steam: %s , Teamkills in this session: %d", i ,stID, g_cache_stID[i][tkills])
                    // ban kick
                }
                else
                {   
                    server_print("__________________ LIST## HIS KILLS is %d of 10!", g_cache_stID[i][tkills])
            ...
            else if(!equal(stID, g_cache_stID[i][STID]))
            {
                if(g_cache_stID[i][zanyat] != 1)
                {
                    ...
                    server_print("__________________ REGISTERED NEW PLAYER to LIST %d : Steam: %s", i ,stID)
                    return PLUGIN_CONTINUE
                }
            }
        }
...
public client_death(killer,victim,wpnindex,hitplace,TK)
{
        ...
        if (get_user_team(killer) == get_user_team(victim) && killer!=victim )
    {
        ...
        server_print("___________________ TEAM KILL id :%d kill id: %d TOTAL TKills: %d", killer ,victim, g_cache_stID[session_id_to_registerlist[killer]][tkills])
        // return PLUGIN_CONTINUE
    }
    /// extantion end
